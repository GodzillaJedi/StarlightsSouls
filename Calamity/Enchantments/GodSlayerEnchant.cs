            ...
            if (player.AddEffect<MechwormEffect>(Item))
            {
                if (player.whoAmI == Main.myPlayer)
                {
                    ...
                    if (player.ownedProjectileCounts[ModContent.ProjectileType<MechwormHead>()] < 1)
                    {
                        ...
                        Projectile.NewProjectileDirect(source_ItemUse, player.Center, -Vector2.UnitY, ModContent.ProjectileType<MechwormHead>(), num, 0f, player.whoAmI).originalDamage = num;
                    }
                }
            }
